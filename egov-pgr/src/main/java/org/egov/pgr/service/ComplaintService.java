...
public class ComplaintService {
	...
	final String[] pendingStatus = { "REGISTERED", "FORWARDED", "PROCESSING",
			"NOTCOMPLETED", "REOPENED" };
	final String[] completedStatus = { "COMPLETED", "WITHDRAWN", "CLOSED" };
	final String[] rejectedStatus = { "REJECTED" };
	final String[] resolvedStatus = { "COMPLETED", "WITHDRAWN", "CLOSED",
			"REJECTED" };
	@Autowired
	...
	public Complaint createComplaint(final Complaint complaint)
		...
		if (complaint.getLocation() == null && complaint.getLat() != 0.0
			...
			try {
				final Long bndryId = boundaryService
						.getBoundaryIdFromShapefile(complaint.getLat(),
								complaint.getLng());
		...
		complaint
				...
				.withComments(
						"Grievance registered with Complaint Number : "
								+ complaint.getCrn())
		...
		if (null != complaint.getComplaintType()
				&& null != complaint.getComplaintType().getDepartment())
			complaint.setDepartment(complaint.getComplaintType()
					.getDepartment());
	...
	public Complaint update(final Complaint complaint,
		...
		final Role goRole = roleService
				.getRoleByName(PGRConstants.GO_ROLE_NAME);
		String userName = null;

		// Temproary solution
		if (complaint.getComplainant().getUserDetail().getType()
				.equals(UserType.CITIZEN))
			userName = complaint.getComplainant().getName();
		else
			userName = complaint.getComplainant().getName() + DELIMITER_COLON
		...
		if (complaint.getStatus().getName()
			...
			complaint.setDepartment(complaint.getAssignee().getDeptDesig()
					.getDepartment());
			LOG.debug("Terminating Grievance Workflow");
			// if (!securityUtils.getCurrentUser().getRoles().contains(goRole))
			if (!complaint.getComplainant().getUserDetail().getRoles()
					.contains(goRole))
			...
			final Position owner = positionMasterService
					.getPositionById(approvalPosition);
	...
	private String getDetailedMessage(final Complaint savedComplaint) {
		...
		detailedMessage.append("Grievance No. ")
				.append(savedComplaint.getCrn()).append(" regarding ")
				.append(savedComplaint.getComplaintType().getName())
				.append(" in ").append(savedComplaint.getStatus().getName())
				.append(" status.");
	...
	public List<Hashtable<String, Object>> getHistory(final Complaint complaint) {
		...
		map.put(COMMENT, state.getComments() != null ? state.getComments()
				: StringUtils.EMPTY);
		if (state.getLastModifiedBy().getType().equals(UserType.CITIZEN)
			...
			map.put(UPDATEDBY,
					state.getSenderName() != null ? state.getSenderName()
							: state.getLastModifiedBy().getUsername()
									+ DELIMITER_COLON
									+ state.getLastModifiedBy().getName());
		...
		if (null != user) {
			...
			final List<Assignment> assignmentList = assignmentService
					.getAssignmentsForPosition(ownerPosition.getId(),
							new Date());
			user = !assignmentList.isEmpty() ? assignmentList.get(0)
					.getEmployee() : null;
			map.put(USER, null != user ? user.getUsername() + DELIMITER_COLON
					+ user.getName() + DELIMITER_COLON
					+ ownerPosition.getDeptDesig().getDesignation().getName()
					: NOASSIGNMENT + DELIMITER_COLON + ownerPosition.getName());
			map.put(USERTYPE, null != user ? user.getType() : StringUtils.EMPTY);
			map.put(DEPT,
					null != ownerPosition.getDeptDesig().getDepartment() ? ownerPosition
		...
		if (!complaint.getStateHistory().isEmpty()
			...
			final List<StateHistory> complaintStateHistory = complaint
					.getStateHistory();
			...
			for (final StateHistory stateHistory : complaintStateHistory) {
				...
				HistoryMap.put(
						COMMENT,
						stateHistory.getComments() != null ? stateHistory
								.getComments() : StringUtils.EMPTY);
				if (stateHistory.getLastModifiedBy().getType()
						.equals(UserType.CITIZEN)
						|| stateHistory.getLastModifiedBy().getType()
								.equals(UserType.SYSTEM))
					HistoryMap.put(UPDATEDBY, complaint.getComplainant()
							.getName());
				else
					HistoryMap.put(UPDATEDBY, stateHistory.getSenderName());
				HistoryMap.put(UPDATEDUSERTYPE, stateHistory
						.getLastModifiedBy().getType());
				HistoryMap.put(STATUS, stateHistory.getValue());
				final Position owner = stateHistory.getOwnerPosition();
				user = stateHistory.getOwnerUser();
				if (null != user) {
					HistoryMap.put(USER, user.getUsername() + DELIMITER_COLON
							+ user.getName());
					...
					final List<Assignment> assignments = assignmentService
							.getAssignmentsForPosition(owner.getId(),
									new Date());
					user = !assignments.isEmpty() ? assignments.get(0)
							.getEmployee() : null;
					HistoryMap.put(USER, null != user ? user.getUsername()
							+ DELIMITER_COLON + user.getName()
							+ DELIMITER_COLON
							+ owner.getDeptDesig().getDesignation().getName()
							: NOASSIGNMENT + DELIMITER_COLON + owner.getName());
					HistoryMap.put(USERTYPE, null != user ? user.getType()
							: StringUtils.EMPTY);
					HistoryMap.put(DEPT, null != owner.getDeptDesig()
	...
	public void sendEmailandSms(final Complaint complaint) {
		final String formattedCreatedDate = new SimpleDateFormat(
				"dd/MM/yyyy HH:mm").format(complaint.getCreatedDate());
		...
		final StringBuffer emailSubject = new StringBuffer()
				.append("Registered Grievance -").append(complaint.getCrn())
				.append(" successfuly");
		final StringBuffer smsBody = new StringBuffer()
				.append("Your grievance for ")
				.append(complaint.getComplaintType().getName())
				.append(" has been registered successfully with tracking number (")
				.append(complaint.getCrn())
...
//		sendMessage(complaint, emailSubject);
		...
		if (null != owner && null != owner.getDeptDesig()) {
			final List<Assignment> assignments = assignmentService
					.getAssignmentsForPosition(owner.getId(), new Date());
			if (!assignments.isEmpty()) {
				final User user = assignments.get(0).getEmployee();
				if (null != user) {
					final StringBuffer smsBodyOfficial = new StringBuffer()
							.append("New Grievance for ")
							.append(complaint.getComplaintType().getName())
							.append(" is registered by ")
					...
					if (complaint.getLatlngAddress() != null)
						smsBodyOfficial.append(", "
								+ complaint.getLatlngAddress());
					else
						smsBodyOfficial
								.append(complaint.getChildLocation() != null ? ", "
										+ complaint.getChildLocation()
												.getName()
										: StringUtils.EMPTY);
					smsBodyOfficial
							.append(complaint.getLandmarkDetails() != null ? ", "
									+ complaint.getLandmarkDetails()
									: StringUtils.EMPTY);
					messagingService.sendSMS(user.getMobileNumber(),
							smsBodyOfficial.toString());
				}
			}
		}
	}
	...
	public void sendSmsOnCompletion(final Complaint complaint) {
		final StringBuffer smsBody = new StringBuffer()
				.append("Your Grievance regarding ")
				.append(complaint.getComplaintType().getName())
	...
	public void sendSmsToOfficials(final Complaint complaint) {
		...
		senderName = complaint.getState().getSenderName()
				.contains(DELIMITER_COLON) ? complaint.getState()
				.getSenderName().split(DELIMITER_COLON)[1] : complaint
				.getState().getSenderName();
		if (null != owner && null != owner.getDeptDesig()) {
			final User user = eisCommonService.getUserForPosition(
					owner.getId(), new Date());
			if (null != user) {
				final StringBuffer smsBodyOfficial = new StringBuffer()
						.append(user.getName() + ", ")
				...
				smsBodyOfficial
						.append(complaint.getLandmarkDetails() != null ? ", "
								+ complaint.getLandmarkDetails()
								: StringUtils.EMPTY);
				smsBodyOfficial.append(" handled by " + senderName
	...
	public Page<Complaint> getLatest(final int page, final int pageSize) {
		...
		return complaintRepository.findByLatestComplaint(securityUtils
				.getCurrentUser(), new PageRequest(offset, pageSize));
	}

	public Page<Complaint> getMyComplaint(final int page, final int pageSize) {
		final int offset = page - 1;
		return complaintRepository.findByMyComplaint(securityUtils
				.getCurrentUser(), new PageRequest(offset, pageSize));
	}

	public List<Complaint> getNearByComplaint(final int page, final float lat,
			final float lng, final int distance, final int pageSize) {
		final Long offset = new Long((page - 1) * pageSize);
		final Long limit = new Long(pageSize + 1);
		return complaintRepository.findByNearestComplaint(securityUtils
				.getCurrentUser().getId(), new Float(lat), new Float(lng),
				new Long(distance), limit, offset);
	}
	...
	public String getEmailBody(final Complaint complaint) {
		final String formattedCreatedDate = new SimpleDateFormat(
				"dd/MM/yyyy HH:mm").format(complaint.getCreatedDate());
		final StringBuffer emailBody = new StringBuffer()
				.append(" %0D%0A Grievance Details -  %0D%0A %0D%0A CRN - ")
				.append(complaint.getCrn()).append(" %0D%0A Grievance Type -")
				.append(complaint.getComplaintType().getName());
		if (complaint.getDepartment() != null)
			emailBody.append("  %0D%0A Grievance department  - ").append(
					complaint.getDepartment().getName());
		if (complaint.getComplainant().getName() != null)
			emailBody.append("  %0D%0A Complainant name - ").append(
					complaint.getComplainant().getName());
		if (complaint.getComplainant().getMobile() != null)
			emailBody.append("  %0D%0A Complainant mobile number - ").append(
					complaint.getComplainant().getMobile());
		if (complaint.getLocation() != null)
			emailBody.append("  %0D%0A Location details - ").append(
					complaint.getLocation().getName());

		emailBody.append(" %0D%0A Grievance details - ")
				.append(complaint.getDetails())
				.append(" %0D%0A Grievance status -")
				.append(complaint.getStatus().getName())
				.append(" %0D%0A Grievance Registration Date - ")
				.append(formattedCreatedDate);
	...
	public List<Complaint> getPendingGrievances() {
		final User user = securityUtils.getCurrentUser();
		final String[] pendingStatus = { "REGISTERED", "FORWARDED",
				"PROCESSING", "NOTCOMPLETED", "REOPENED" };
		final Criteria criteria = entityManager.unwrap(Session.class)
				.createCriteria(Complaint.class, "complaint")
				.createAlias("complaint.state", "state")
				.createAlias("complaint.status", "status");
		criteria.add(Restrictions.in("status.name", pendingStatus));
		criteria.add(Restrictions.eq("complaint.assignee",
				positionMasterService.getCurrentPositionForUser(user.getId())));
		return criteria.list();
	}

	public Page<Complaint> getMyPendingGrievances(final int page,
			final int pageSize) {
		final int offset = page - 1;
		return complaintRepository.findMyComplaintyByStatus(securityUtils
				.getCurrentUser(), pendingStatus, new PageRequest(offset,
				pageSize));
	}

	public Page<Complaint> getMyCompletedGrievances(final int page,
			final int pageSize) {
		final int offset = page - 1;
		return complaintRepository.findMyComplaintyByStatus(
				securityUtils.getCurrentUser(), completedStatus,
				new PageRequest(offset, pageSize));
	}

	public Page<Complaint> getMyRejectedGrievances(final int page,
			final int pageSize) {
		final int offset = page - 1;
		return complaintRepository.findMyComplaintyByStatus(
				securityUtils.getCurrentUser(), rejectedStatus,
				new PageRequest(offset, pageSize));
	}
	...
	public HashMap<String, Integer> getMyComplaintsCount() {
		...
		complaintsCount.put(COMPLAINT_ALL, complaintRepository
				.getMyComplaintsTotalCount(securityUtils.getCurrentUser())
				.intValue());
		complaintsCount.put(
				COMPLAINT_PENDING,
				complaintRepository.getMyComplaintCountByStatus(
						securityUtils.getCurrentUser(), pendingStatus)
						.intValue());
		complaintsCount.put(
				COMPLAINT_COMPLETED,
				complaintRepository.getMyComplaintCountByStatus(
	...
	public HashMap<String, Integer> getComplaintsTotalCount() {
		final HashMap<String, Integer> complaintsCount = new HashMap<String, Integer>();
		complaintsCount.put(COMPLAINTS_FILED, complaintRepository
				.getTotalComplaintsCount().intValue());
		complaintsCount.put(COMPLAINTS_RESOLVED, complaintRepository
				.getComplaintsTotalCountByStatus(resolvedStatus).intValue());
		complaintsCount.put(COMPLAINTS_UNRESOLVED, complaintRepository
				.getComplaintsTotalCountByStatus(pendingStatus).intValue());
		return complaintsCount;
	}

	public List<Complaint> getOpenComplaints() {
		final List<String> statusList = Arrays.asList("REGISTERED",
				"FORWARDED", "REOPENED", "PROCESSING");
		return complaintRepository.findByStatusNameIn(statusList);
	}

	public List<Complaint> getBySearchInputs(String code, String status,
			String crn, String start_date, String creater_id)
			throws ParseException {

		final Criteria criteria = entityManager.unwrap(Session.class)
				.createCriteria(Complaint.class, "complaint")
				.createAlias("complaint.status", "status")
				.createAlias("complaint.complaintType", "complainttype")
	...
	private void sendMessage(Complaint complaint, StringBuffer emailSubject) {
		...
		producer.send(new ProducerRecord<String, String>(
				"egov-notification-sms", "pgrrest", gson.toJson(smsRequest).toString()));
		producer.flush();
		producer.send(new ProducerRecord<String, String>(
				"egov-notification-email", "pgrrest", gson.toJson(emailRequest).toString()));
		producer.flush();
		producer.close();
	}
}