...
public class BaseVoucherAction extends BaseWorkFlowAction {
	...
	protected void validateCutOffDate(Date vhDate) {
		if (workflowBean.getWorkflowAction().equalsIgnoreCase(FinancialConstants.CREATEANDAPPROVE)) {

			try {
				String localCutoOffDate = getCutoffDateConfigValue();
				Date configuredDate = df.parse(localCutoOffDate);
				if (vhDate.after(configuredDate)) {
					final List<ValidationError> errors = new ArrayList<ValidationError>();
					errors.add(new ValidationError("Create And Approve Not valid", "Create And Approve Not valid"));
					throw new ValidationException(errors);
				}
			} catch (ParseException e) {
				LOGGER.error("Error while parsing cutOffDate");
			}
		}

	}

	protected String getCutoffDateConfigValue() {

		String configuredDate = null;
		try {
			...
			if (cutOffDateconfigValue != null && !cutOffDateconfigValue.isEmpty()) {
				...
				configuredDate = df.format(configuredDBDate);
			} else {
	...
	/*
	 * protected void loadFunction(){ if(headerFields.contains("function") &&
	 * null != voucherHeader.getVouchermis().getFunction()){ List<Fundsource>
	 * fundSourceList =
	 * financingSourceService.getFinancialSourceBasedOnSubScheme(voucherHeader.
	...
	protected HashMap<String, Object> createHeaderAndMisDetails() throws ValidationException {
		...
		if (voucherHeader.getVouchermis().getDepartmentid() != null)
			headerdetails.put(VoucherConstant.DEPARTMENTCODE,
					voucherHeader.getVouchermis().getDepartmentid().getCode());
	...
	protected boolean validateData(final List<VoucherDetails> billDetailslist,
		...
		// Map<String, String> glcodeMap = new HashMap<String, String>();// to
		// support same account code to be used multiple
		// times.
		for (final VoucherDetails voucherDetails : billDetailslist) {
			...
			/*
			 * if
			 * (voucherDetails.getDebitAmountDetail().compareTo(BigDecimal.ZERO)
			 * ==0 &&
			 * voucherDetails.getCreditAmountDetail().compareTo(BigDecimal.ZERO)
			 * == 0 && voucherDetails.getGlcodeDetail().trim().length()==0) {
			 * addActionError(getText("journalvoucher.accdetail.emptyaccrow",new
			 * String[]{""+index})); isValFailed= true ; } else
			 */
			/*
			 * if(null !=
			 * glcodeMap.get(voucherDetails.getGlcodeDetail().trim())){
			 * addActionError(getText("journalvoucher.accdetail.repeated",new
			 * String[]{voucherDetails.getGlcodeDetail()})); isValFailed= true ;
			 * }else{ glcodeMap.put(voucherDetails.getGlcodeDetail().trim(),
			...
			if (voucherDetails.getDebitAmountDetail().compareTo(BigDecimal.ZERO) == 0
				 ...
				 * if(voucherHeader.getIsRestrictedtoOneFunctionCenter()){
				 * functionId =
				 * voucherHeader.getFunctionId()!=null?voucherHeader.
				 * getFunctionId().toString():"0"; }else{
				 */
	...
	protected boolean validateSubledgerDetails(final List<VoucherDetails> billDetailslist,
		...
		final List<Map<String, Object>> subLegAccMap = new ArrayList<Map<String, Object>>(); // this
																								// list
																								// will
																								// contain
																								// the
			...
			if (voucherDetails.getGlcode().getId() != 0) {
				...
				if (null != subledAmtmap.get(voucherDetails.getGlcode().getId() + "-" + function)) {
					final BigDecimal debitTotalAmount = subledAmtmap
							.get(voucherDetails.getGlcode().getId() + "-" + function).add(voucherDetails.getAmount());
